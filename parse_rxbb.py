...
def write_union(file, addr, names, name2members):
    ...
    num = len(names)
    for name in names:
        write_struct(file, name, addr, name2members[name], "    ")
        if num > 1:
            str = '\n'
            file.write(str)
        num -= 1
            ...
            if len(pos) == 2:
                if first_flag:
                    if int(pos[1]) != 0:
                        ...
                        reg_members["reserved"] = int(pos[1])
                        bit_sum += int(pos[1])
                    first_flag = False
                bits = int(pos[0]) - int(pos[1]) + 1
                if log_en:
                    print(pos_text, bits, reg_field)
            else:
                bits = 1
                if first_flag:
                    if int(pos[0]) != 0:
                        # print("not match", name)
                        reg_members["reserved"] = int(pos[0])
                        bit_sum += int(pos[0])
                    first_flag = False
                if log_en:
                    print(pos_text, bits, reg_field)
